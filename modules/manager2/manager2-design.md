...
- Tree: the live component tree below `Server` (services, engines,
  ...
  the manager), and the TLS branch of a connector: SSL host
  configurations with their certificates). Each row: a type badge, the
  name, a state badge for lifecycle components, and a "this app" badge
  on the context hosting this webapp. Nodes expand/collapse; the
...
- *+ Add* (shown for nodes that can have children) opens a modal with
    ...
    manager → sessionIdGenerator; connector → sslHostConfig; sslHostConfig →
    certificate; cluster → channel, deployer, clusterValve, clusterManager,
