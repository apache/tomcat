...
4. **No new Java dependencies** (JDK + existing Tomcat/Jakarta APIs only)
 ...
 5. **i18n** — every user-visible string lives in the
    `org.apache.tomcat.manager2` `LocalStrings` bundle. A request-scoped
    `LocaleFilter` resolves the browser's `Accept-Language` to a per-locale
    `StringManager` (exact match, then language-only, then default), so API
    responses and the `#{key}` placeholders of the login / error / SPA
    templates — substituted server-side by `Html.render`, which also fills
    `{{lang}}` for the `lang` attribute — are localized per request. The
    browser loads the `manager2.ui.*` keys (and only those) from the public
    `GET /i18n` endpoint and renders its strings through `js/i18n.js`.
    Only text inherited from `HTMLManagerServlet` / `HostManagerServlet`
    operations still comes from the existing `org.apache.catalina.manager`
    bundles (already translated to 10 locales). The descriptions of the
    configuration editor are not stored in the code either: they are looked
    up dynamically by attribute name (`manager2.attr.<scope>.<name>` for
    component and JNDI entry attributes, `manager2.param.<name>` —
    optionally scoped as `manager2.param.<factoryKey>.<name>` — for the
    factory options of a JNDI resource); for the standard attributes these
    bundle messages override the English text of the MBean descriptors
    shipped with the container, and `TestManager2Descriptions` fails when a
    seeded copy drifts from the descriptor it was taken from.
...
- Tree: the live component tree below `Server` (services, engines,
  ...
  loader, cookie processor; the manager's session id generator under
   ...
   configurations with their certificates and pre-shared keys), and the
   upgrade protocols of a connector). Each row: a type badge, the name, a state badge
...
- *+ Add* (shown for nodes that can have children) opens a modal with
    ...
    upgradeProtocol; sslHostConfig → certificate, preSharedKey;
    cluster → channel,
...
- *TLS*: adding an `sslHostConfig` to a running connector switches it
  ...
  changes so no stale (in)secure channel is handed out. A pre-shared
  key (`preSharedKey`, edited as `identity` / hexadecimal `key` /
  `digest`) is a child of an SSL host configuration alongside the
  certificates; adding, editing or removing one re-validates a running,
  TLS enabled connector and rolls back on failure, and unlike a
  certificate a pre-shared key is never required, so the last one can be
  removed freely. Pre-shared keys are only consumed by the OpenSSL
  (native and FFM) TLS implementation.
...
modules/manager2/
  ...
  src/main/java/org/apache/tomcat/manager2/
    ...
    I18nServlet.java          GET /i18n — serves the manager2.ui.* bundle keys
...
11. **Configuration** (`TestManager2Config`): tree shape (the service,
    ...
    (including a just-added alias) and keeps a timestamped backup.
     ...
     plain HTTP again, verified with a plain request). Pre-shared keys
     (on the same TLS connector): add a `preSharedKey` (guards: missing
     identity/key → 400 `MISSING_FIELD`, non-hexadecimal key → 400
     `INVALID_VALUE`, duplicate identity → 409 `DUPLICATE`), its node
     detail (`identity`, hexadecimal `key`, `digest`), editing the
     digest/key, and removal (the last key can be removed); the
     `<PreSharedKey>` element appears in the stored `server.xml` and
     round-trips through the digester. Realms: the
