...
--bundle: Add the spcified path (directory or bundle) as a webapp (if no path has been specified, it will be the root webapp)
```
...
```
java -Dcatalina.base=. -Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager -Djava.util.logging.config.file=conf/logging.properties -jar target/tomcat-stuffed-1.0.jar --bundle myrootwebapp
```

### Command line example with three webapps

```
java -Dcatalina.base=. -Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager -Djava.util.logging.config.file=conf/logging.properties -jar target/tomcat-stuffed-1.0.jar --bundle myrootwebapp --path /path1 --war mywebapp1 --path /path2 --war mywebapp2
```
