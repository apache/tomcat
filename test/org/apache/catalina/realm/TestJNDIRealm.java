...
public class TestJNDIRealm {
    ...
    private JNDIRealm buildRealm(String password) throws javax.naming.NamingException, NoSuchFieldException, IllegalAccessException, NoSuchAlgorithmException, LifecycleException {
    ...
    private NamingEnumeration mockSearchResults(String password) throws NamingException {
        NamingEnumeration searchResults = createNiceMock(NamingEnumeration.class);
        expect(searchResults.hasMore())
                .andReturn(true)
                .andReturn(false)
                .andReturn(true)
                .andReturn(false);
        expect(searchResults.next())
                .andReturn(new SearchResult("ANY RESULT", "", new BasicAttributes(USER_PASSWORD_ATTR, password)))
                .times(2);
        EasyMock.replay(searchResults);
        return searchResults;
    }

    private DirContext mockDirContext(NamingEnumeration namingEnumeration) throws NamingException {
